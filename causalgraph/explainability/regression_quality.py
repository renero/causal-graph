...
class RegQuality(BaseEstimator):
    ...
    def predict(
        ...
        gamma_indices = [idx for idx, score in enumerate(scores) \
            if stats.gamma.pdf(score, a=gamma_shape, scale=gamma_scale) <= threshold]
    
        # median = np.median(scores)
        # iqr = stats.iqr(scores)
        # outliers_indices = [idx for idx, score in enumerate(scores) \
        #     if np.abs(score-median) > iqr_multiplier*iqr]
        outliers_indices = RegQuality._mad_criteria(scores, verbose=verbose)
    
        return set(gamma_indices).intersection(outliers_indices)
    
    @staticmethod
    def _iqr_criteria(scores, multiplier:float=2.5, verbose=False) -> Set[int]:
        """
        Returns indices of outliers in the given scores, using the 2.5*IQR method.
        ...
        verbose: bool
            Whether to print the score and IQR criteria for each score
        ...
        median = np.median(scores)
        outliers_indices = np.where(np.abs(scores-median) > multiplier*iqr)
        if verbose:
            print(f"Scorings (MSE): {scores}")
            print(f"{multiplier:.1f}·IQR: {multiplier*iqr:.4f}"
                f" - Median.: {median:.4f}")
            print(f"Outliers indices: {outliers_indices}")
            
        return set(outliers_indices[0])
